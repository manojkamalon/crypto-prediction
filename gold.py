...
st.set_page_config(page_title="XAU/USD Price Prediction", layout="wide")

# Title and description
st.title("📈 XAU/USD Price Prediction App")
st.write("""
This app predicts the future price of XAU/USD (Gold/US Dollar) using historical data and machine learning models.
""")
...
@st.cache_data
def load_data():
    # Download XAU/USD data from Yahoo Finance
...
ax.plot(data['Date'], data['Close'], label="Close Price", color="gold")
ax.set_title("XAU/USD Close Price Over Time")
ax.set_xlabel("Date")
...
if model_choice == "Linear Regression":
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(data['Date'], data['Close'], label="Historical Close Price", color="gold")
    ax.plot(future_dates, predictions, label="Predicted Close Price", color="blue", linestyle="--")
    ax.set_title("XAU/USD Price Prediction")
    ax.set_xlabel("Date")
    ax.set_ylabel("Price (USD)")
    ax.legend()
    st.pyplot(fig)
